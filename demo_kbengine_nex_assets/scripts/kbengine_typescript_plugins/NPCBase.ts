/*
	...
	Please inherit this module, such as: (class NPC extends NPCBase)
	tools = kbcmd
*/
...
import { EntityBaseEntityCall_NPCBase, EntityCellEntityCall_NPCBase } from './EntityCallNPCBase';

// defined in */scripts/entity_defs/NPC.def
// Please inherit and implement "class NPC extends NPCBase"
export abstract class NPCBase extends Entity{
	public baseEntityCall:EntityBaseEntityCall_NPCBase | null = null;
	public cellEntityCall:EntityCellEntityCall_NPCBase | null = null;
	...
	public override onRemoteMethodCall(stream:MemoryStream){
		let sm:ScriptModule = EntityDef.moduledefs["NPC"];
	...
	public override callPropertysSetMethods(){
		...
		let oldval_entityNO:number = this.entityNO;
		let prop_entityNO:Property = pdatas[4];
		...
		let oldval_modelID:number = this.modelID;
		let prop_modelID:Property = pdatas[5];
		...
		let oldval_modelScale:number = this.modelScale;
		let prop_modelScale:Property = pdatas[6];
		...
		let oldval_moveSpeed:number = this.moveSpeed;
		let prop_moveSpeed:Property = pdatas[7];
		...
		let oldval_name:string = this.name;
		let prop_name:Property = pdatas[8];
		if(prop_name.isBase()){
			if(this.inited && !this.inWorld)
				this.onNameChanged(oldval_name);
		}
		...
		let oldval_uid:number = this.uid;
		let prop_uid:Property = pdatas[9];
		if(prop_uid.isBase()){
			if(this.inited && !this.inWorld)
				this.onUidChanged(oldval_uid);
		}
		...
		let oldval_utype:number = this.utype;
		let prop_utype:Property = pdatas[10];
