/*
	...
	Please inherit this module, such as: (class Gate extends GateBase)
	tools = kbcmd
*/
...
import { EntityBaseEntityCall_GateBase, EntityCellEntityCall_GateBase } from './EntityCallGateBase';

// defined in */scripts/entity_defs/Gate.def
// Please inherit and implement "class Gate extends GateBase"
export abstract class GateBase extends Entity{
	public baseEntityCall:EntityBaseEntityCall_GateBase | null = null;
	public cellEntityCall:EntityCellEntityCall_GateBase | null = null;
	...
	public override onRemoteMethodCall(stream:MemoryStream){
		let sm:ScriptModule = EntityDef.moduledefs["Gate"];
	...
	public override callPropertysSetMethods(){
		...
		let oldval_name:string = this.name;
		let prop_name:Property = pdatas[7];
		if(prop_name.isBase()){
			if(this.inited && !this.inWorld)
				this.onNameChanged(oldval_name);
		}
		...
		let oldval_uid:number = this.uid;
		let prop_uid:Property = pdatas[8];
		if(prop_uid.isBase()){
			if(this.inited && !this.inWorld)
				this.onUidChanged(oldval_uid);
		}
		...
		let oldval_utype:number = this.utype;
		let prop_utype:Property = pdatas[9];
