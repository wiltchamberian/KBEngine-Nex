	...
	*/
	public class MemoryStream : ObjectPool<MemoryStream>
    {
		...
		public KBVector2 readPackXZ()
		{
			...
			KBVector2 vec = new KBVector2(xPackData.fv, zPackData.fv);
			return vec;
		}
		...
		public void writeBlob(byte[] v)
		{
			...
			if(size + 4 > space())
			{
				KBELog.ERROR_MSG("memorystream::writeBlob: no free!");
				return;
			}
		...
		public void writeString(string v)
		{
			if(v.Length > space())
			{
				KBELog.ERROR_MSG("memorystream::writeString: no free!");
				return;
			}
