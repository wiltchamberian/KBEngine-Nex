namespace KBEngine
{
	...
	*/
    public class Entity 
    {
		...
		public KBVector3 _entityLastLocalPos = new KBVector3(0f, 0f, 0f);
		public KBVector3 _entityLastLocalDir = new KBVector3(0f, 0f, 0f);
		
    	public Int32 id = 0;
		public string className = "";
		public KBVector3 position = new KBVector3(0.0f, 0.0f, 0.0f);
		public KBVector3 direction = new KBVector3(0.0f, 0.0f, 0.0f);
		...
		*/
		...
		{			
			...
			{
				KBELog.ERROR_MSG(className + "::baseCall(" + methodname + "), currserver=!" + KBEngineApp.app.currserver);  
				return;
			}
			...
			{
				KBELog.ERROR_MSG("entity::baseCall:  entity-module(" + className + ") error, can not find from EntityDef.moduledefs");
				return;
			}
			...
			{
				KBELog.ERROR_MSG(className + "::baseCall(" + methodname + "), not found method!");  
				return;
			}
			...
			{
				KBELog.ERROR_MSG(className + "::baseCall(" + methodname + "): args(" + (arguments.Length) + "!= " + method.args.Count + ") size is error!");  
				return;
			}
			...
			{
				KBELog.ERROR_MSG(className + "::baseCall(method=" + methodname + "): args is error(" + e.Message + ")!");  
		...
		public void cellCall(string methodname, params object[] arguments)
		{
			if(KBEngineApp.app.currserver == "loginapp")
			{
				KBELog.ERROR_MSG(className + "::cellCall(" + methodname + "), currserver=!" + KBEngineApp.app.currserver);  
				return;
			}
			...
			if(!EntityDef.moduledefs.TryGetValue(className, out module))
			{
				KBELog.ERROR_MSG("entity::cellCall:  entity-module(" + className + ") error, can not find from EntityDef.moduledefs!");
				return;
			}
			...
			{
				KBELog.ERROR_MSG(className + "::cellCall(" + methodname + "), not found method!");  
				return;
			}
			...
			if(arguments.Length != method.args.Count)
			{
				KBELog.ERROR_MSG(className + "::cellCall(" + methodname + "): args(" + (arguments.Length) + "!= " + method.args.Count + ") size is error!");  
				return;
			}
			...
			{
				KBELog.ERROR_MSG(className + "::cellCall(" + methodname + "): no cell!");  
				return;
			}
			...
			{
				KBELog.ERROR_MSG(className + "::cellCall(" + methodname + "): args is error(" + e.Message + ")!");  
		...
		{
			// KBELog.DEBUG_MSG(className + "::enterWorld(" + getDefinedProperty("uid") + "): " + id); 
			inWorld = true;
			...
			{
				KBELog.ERROR_MSG(className + "::onEnterWorld: error=" + e.ToString());
			}
		...
		{
			// KBELog.DEBUG_MSG(className + "::leaveWorld: " + id); 
			inWorld = false;
			...
			{
				KBELog.ERROR_MSG(className + "::onLeaveWorld: error=" + e.ToString());
			}
		...
		{
			// KBELog.DEBUG_MSG(className + "::leaveSpace: " + id); 
			inWorld = false;
			...
			{
				KBELog.ERROR_MSG(className + "::onLeaveSpace: error=" + e.ToString());
			}
		...
		public virtual void onPositionChanged(KBVector3 oldValue)
		{
			//KBELog.DEBUG_MSG(className + "::set_position: " + oldValue + " => " + v); 
		...
		public virtual void onDirectionChanged(KBVector3 oldValue)
		{
			//KBELog.DEBUG_MSG(className + "::set_direction: " + oldValue + " => " + v); 
