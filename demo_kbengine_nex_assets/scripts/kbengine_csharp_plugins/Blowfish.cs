namespace KBEngine
{
    ...
    public class Blowfish
    {
        ...
        private void init()
        {
            ...
            {
                _isGood = false;
                KBELog.ERROR_MSG("Blowfish::init: invalid length " + _key.Length);
                return;
            }
        ...
        public void encipher(byte[] data, int length)
        {
            ...
            for (int i = 0; i < length; i += 8)
            {
                if (prevBlock != 0)
                {
                    UInt64 oldValue = BitConverter.ToUInt64(data, i);
                    UInt64 ret = oldValue ^ prevBlock;
                    prevBlock = oldValue;

                    data[i] = (byte)ret;
                    data[i + 1] = (byte)(ret >> 0x8);
                    data[i + 2] = (byte)(ret >> 0x10);
                    data[i + 3] = (byte)(ret >> 0x18);
                    data[i + 4] = (byte)(ret >> 0x20);
                    data[i + 5] = (byte)(ret >> 0x28);
                    data[i + 6] = (byte)(ret >> 0x30);
                    data[i + 7] = (byte)(ret >> 0x38);
                }
                else
                {
        ...
        public void decipher(byte[] data, int startIndex, int length)
        {
            ...
            for (int i = 0; i < length; i += 8)
            {
                ...
                data[index + 2] = (byte)(xl >> 8);
                data[index + 3] = (byte)(xl);
                data[index + 4] = (byte)(xr >> 24);
                data[index + 5] = (byte)(xr >> 16);
                ...
                if(prevBlock != 0)
                {
                    ...
                    data[index] = (byte)ret;
                    data[index + 1] = (byte)(ret >> 0x08);
                    data[index + 2] = (byte)(ret >> 0x10);
                    data[index + 3] = (byte)(ret >> 0x18);
                    data[index + 4] = (byte)(ret >> 0x20);
                    data[index + 5] = (byte)(ret >> 0x28);
                    data[index + 6] = (byte)(ret >> 0x30);
                    data[index + 7] = (byte)(ret >> 0x38);
                }

                prevBlock = BitConverter.ToUInt64(data, index);
            }
        }
