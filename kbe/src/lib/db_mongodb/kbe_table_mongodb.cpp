...
namespace KBEngine {
	...
	bool KBEAccountTableMongodb::setFlagsDeadline(DBInterface* pdbi, const std::string& name, uint32 flags, uint64 deadline)
	{
		...
		bson_append_utf8(&query, accountName.c_str(), static_cast<int>(accountName.size()), name.c_str(), static_cast<int>(name.size()));
	...
	bool KBEAccountTableMongodb::updatePassword(DBInterface* pdbi, const std::string& name, const std::string& password)
	{
		...
		bson_t query;
		bson_init(&query);
		bson_append_utf8(&query, "entityDBID", static_cast<int>(strlen("entityDBID")), name.c_str(), static_cast<int>(name.size()));
	...
	bool KBEAccountTableMongodb::logAccount(DBInterface* pdbi, ACCOUNT_INFOS& info)
	{
		...
		bson_append_utf8(&options, "bindata", (int)strlen("bindata"), info.datas.data(), static_cast<int>(info.datas.size()));
