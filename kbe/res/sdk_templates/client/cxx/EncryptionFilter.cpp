...
std::string generateRandomKey(size_t len)
{
	...
	pBlowFishKey_(NULL)
{
	...
	key_ = generateRandomKey(20).c_str();
	keySize_ = key_.size();

	init();
}

BlowfishFilter::BlowfishFilter(const KBString & key) :
	isGood_(false),
	pPacket_(new MemoryStream()),
	pEncryptStream_(new MemoryStream()),
	packetLen_(0),
...
BlowfishFilter::~BlowfishFilter()
{
	...
	auto* bfKey = static_cast<Blowfish*>(pBlowFishKey_);
	KBE_SAFE_RELEASE(bfKey);
}

bool BlowfishFilter::init()
{
	pBlowFishKey_ = new Blowfish();

	if (MIN_KEY_SIZE <= keySize_ && keySize_ <= MAX_KEY_SIZE)
	{

		// BF_set_key(this->pBlowFishKey(), key_.length(), reinterpret_cast<const unsigned char*>(TCHAR_TO_ANSI(key_)));
		pBlowFishKey_->setKey(reinterpret_cast<const uint8_t*>(TCHARToANSI(key_.c_str()).data()), key_.length());
		isGood_ = true;
	}
	else
	{
...
void BlowfishFilter::encrypt(uint8 *buf, MessageLengthEx len)
{
	...
	for (uint32 i = 0; i < len; i += BLOCK_SIZE)
	{
		...
		uint32_t left = Blowfish::read_u32_be(data + i);
		uint32_t right = Blowfish::read_u32_be(data + i + 4);
		pBlowFishKey_->encryptBlock(left, right);
		Blowfish::write_u32_be(data + i, left);
		Blowfish::write_u32_be(data + i + 4, right);
...
void BlowfishFilter::decrypt(uint8 *buf, MessageLengthEx len)
{
	...
	for (uint32 i = 0; i < len; i += BLOCK_SIZE)
	{
		...
		// read two 32-bit words from data[i..i+7] big-endian
		uint32_t left = Blowfish::read_u32_be(data + i);
		uint32_t right = Blowfish::read_u32_be(data + i + 4);
		pBlowFishKey_->encryptBlock(left, right);
		Blowfish::write_u32_be(data + i, left);
