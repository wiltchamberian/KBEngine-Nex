...
public:
	...
	std::shared_ptr<NetworkInterfaceBase> pNetworkInterface() const {
	...
	const KBArray<uint8>& serverdatas() const
	{
		return serverdatas_;
	}

	void entityServerPos(const KBVector3f& pos)
	{
	...
	static bool validEmail(const KBString& strEmail);

	/*
		通过错误id得到错误描述
	*/
	KBString serverErr(uint16 id);
	...
	*/
	bool login(const KBString& username, const KBString& password, const KBArray<uint8>& datas);
	virtual void onConnectCallback(KBString ip, uint16 port, bool success, int userdata) override;
	...
	*/
	...
	/*
		创建账号
	*/
	bool createAccount(const KBString& username, const KBString& password, const KBArray<uint8>& datas);

	/*
		重置密码, 通过loginapp
	*/
	void resetPassword(const KBString& username);
	void onOpenLoginapp_resetpassword();
	void onConnectTo_resetpassword_callback(KBString ip, uint16 port, bool success);
	void Client_onReqAccountResetPasswordCB(uint16 failcode);

	/*
		绑定Email，通过baseapp
	*/
	void bindAccountEmail(const KBString& emailAddress);
	void Client_onReqAccountBindEmailCB(uint16 failcode);

	/*
		设置新密码，通过baseapp， 必须玩家登录在线操作所以是baseapp。
	*/
	void newPassword(const KBString& old_password, const KBString& new_password);
	...
	*/
	void reloginBaseapp();
	void onReloginTo_baseapp_callback(KBString ip, uint16 port, bool success);
	...
	*/
	...
	/*
		服务端通知创建一个角色
	*/
	void Client_onCreatedProxies(uint64 rndUUID, int32 eid, KBString& entityType);
	...
	*/
	...
	KBString getSpaceData(const KBString& key);

	/*
		服务端初始化客户端的spacedata， spacedata请参考API
	*/
	void Client_setSpaceData(uint32 spaceID, const KBString& key, const KBString& value);

	/*
		服务端删除客户端的spacedata， spacedata请参考API
	*/
	void Client_delSpaceData(uint32 spaceID, const KBString& key);

	/*
		服务端通知流数据下载开始
		请参考API手册关于onStreamDataStarted
	*/
	void Client_onStreamDataStarted(int16 id, uint32 datasize, KBString descr);
...
private:
	...
	void onConnectTo_loginapp_login_callback(KBString ip, uint16 port, bool success);
	void onLogin_loginapp();

	void login_baseapp(bool noconnect);
	void onConnectTo_baseapp_callback(KBString ip, uint16 port, bool success);
	...
	void addSpaceGeometryMapping(uint32 uspaceID, const KBString& respath);

	void resetpassword_loginapp(bool noconnect);

	void createAccount_loginapp(bool noconnect);
	void onOpenLoginapp_createAccount();
	void onConnectTo_loginapp_create_callback(KBString ip, uint16 port, bool success);
...
public:
	...
	const KBString& entity_type() const {
		return entity_type_;
	}

	const KBString& serverVersion() const {
		return serverVersion_;
	}

	const KBString& clientVersion() const {
		return clientVersion_;
	}

	const KBString& serverScriptVersion() const {
		return serverScriptVersion_;
	}

	const KBString& clientScriptVersion() const {
		return clientScriptVersion_;
	}

	const KBString& component() const {
		return component_;
	}

	const KBString& currserver() const {
		return currserver_;
	}

	const KBString& currstate() const {
		return currstate_;
	}

	typedef KBMap<ENTITY_ID, Entity*> ENTITIES_MAP;
...
protected:
	...
	std::shared_ptr<NetworkInterfaceBase> pNetworkInterface_;
	...
	// 当前状态
	KBString currserver_;
	KBString currstate_;

	// 服务端下行以及客户端上行用于登录时处理的账号绑定的二进制信息
	// 该信息由用户自己进行扩展
	KBArray<uint8> serverdatas_;
	KBArray<uint8> clientdatas_;

	// 通信协议加密，blowfish协议
	KBArray<uint8> encryptedKey_;

	// 服务端与客户端的版本号以及协议MD5
	KBString serverVersion_;
	KBString clientVersion_;
	KBString serverScriptVersion_;
	KBString clientScriptVersion_;
	KBString serverProtocolMD5_;
	KBString serverEntitydefMD5_;

	// 当前玩家的实体id与实体类别
	uint64 entity_uuid_;
	ENTITY_ID entity_id_;
	KBString entity_type_;
	
	// 这个参数的选择必须与kbengine_defs.xml::cellapp/aliasEntityID的参数保持一致
	bool useAliasEntityID_;

	KBArray<Entity*> controlledEntities_;

	// 当前服务端最后一次同步过来的玩家位置
	KBVector3f entityServerPos_;

	// space的数据，具体看API手册关于spaceData
	// https://github.com/kbengine/kbengine/tree/master/docs/api
	KBMap<KBString, KBString> spacedatas_;
	...
	KBArray<ENTITY_ID> entityIDAliasIDList_;
	KBMap<ENTITY_ID, MemoryStream*> bufferedCreateEntityMessages_;
	...
	std::shared_ptr<hv::EventLoopThread> mainLoopThread_;
	hv::EventLoopPtr mainLoop_;
