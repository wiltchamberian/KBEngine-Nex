...
Entity::~Entity()
{
	DEBUG_MSG("%s::~%s() %d", className_.c_str(), className_.c_str(), id());
}
...
void Entity::enterWorld()
{
	...
	auto pEventData = std::make_shared<UKBEventData_onEnterWorld>();
...
void Entity::leaveWorld()
{
	...
	auto pEventData = std::make_shared<UKBEventData_onLeaveWorld>();
	pEventData->entityID = id();
	pEventData->spaceID = KBEngineApp::getSingleton().spaceID();
	pEventData->isPlayer = isPlayer();
...
void Entity::enterSpace()
{
	...
	auto pEventData = std::make_shared<UKBEventData_onEnterSpace>();
	pEventData->entityID = id();
	pEventData->spaceID = KBEngineApp::getSingleton().spaceID();
	// KBPos2UE4Pos(pEventData->position, position);
	pEventData->direction = direction;
...
void Entity::leaveSpace()
{
	...
	auto pEventData = std::make_shared<UKBEventData_onLeaveSpace>();
	pEventData->entityID = id();
	pEventData->spaceID = KBEngineApp::getSingleton().spaceID();
	pEventData->isPlayer = isPlayer();
...
void Entity::onPositionChanged(const KBVector3f& oldValue)
{
	...
	if(inWorld())
	{
		...
		auto pEventData = std::make_shared<UKBEventData_set_position>();
		// KBPos2UE4Pos(pEventData->position, position);
		pEventData->entityID = id();
		pEventData->moveSpeed = velocity_;
