...
inline bool IsLittleEndian()
{
	...
	uint8_t* p = reinterpret_cast<uint8_t*>(&test);
	return p[0] == 0x02;
}
...
// ==========================
// EndianConvertReverse: 反向转换（小端系统需要翻转）
// ==========================
template<typename T>
...
public:
	...
	MemoryStream &operator>>(KBString& value)
	{
		char cc[2];
		cc[0] = 0;
		cc[1] = 0;
		...
		while (length() > 0)
		{
			...
			KBString ch = ANSIToTCHAR(cc);  // 转成 KBCHAR
			value.Append(ch);               // 追加到 KBString
	...
	uint32 readBlob(KBArray<uint8>& datas)
	{
		if (length() <= 0)
			return 0;
	...
	uint32 readUTF8String(KBString& str)
	{
		if (length() <= 0)
			return 0;
		...
		// 结尾标志
		datas.Add(0);
		str = KBString(UTF8ToTCHAR(reinterpret_cast<const char*>(datas.GetData())));
		// str = KBString(UTF8ToTCHAR(datas.GetData()));
		return rsize;
	}

	KBString readUnicode()
	{
		KBString str;
		readUTF8String(str);
		return str;
	...
	KBVector2f readVector2()
	{
		float X = readFloat();
		float Y = readFloat();
		return KBVector2f(X, Y);
	}

	KBVector3f readVector3()
	{
		float X = readFloat();
		float Y = readFloat();
		float Z = readFloat();
		return KBVector3f(X, Y, Z);
	}

	KBVector4f readVector4()
	{
		float X = readFloat();
		float Y = readFloat();
		float Z = readFloat();
		float W = readFloat();
		return KBVector4f(X, Y, Z, W);
	}

	KBArray<uint8> readPython()
	{
		return readBlob();
	}
	...
	void appendBlob(const KBArray<uint8>& datas)
	{
	...
	// void appendUTF8String(const KBString& str)
	// {
	...
	void appendUTF8String(const KBString& str)
	{
		// 1. 将 KBString 转为 std::string（UTF-8）
		std::string utf8Str;
//
// #ifdef _WIN32
// 		// Windows wchar_t -> UTF-8
// 		std::wstring_convert<std::codecvt_utf8_utf16<wchar_t>> convert;
// 		utf8Str = convert.to_bytes(str);
// #else
// 		// Linux/macOS char，假设 KBString 已经是 UTF-8
// 		utf8Str = str;
// #endif
		...
		// 3. 写内容
		if (len > 0)
			append(reinterpret_cast<const uint8_t*>(utf8Str.data()), len);
	}
	...
	void writeBlob(const KBArray<uint8>& datas)
	{
		appendBlob(datas);
	}
		
	void writeString(const KBString& v)
	{
		if(v.length() > (int32)space())
		{
	...
	void writeVector2(const KBVector2f& v)
	{
		writeFloat(v.x);
		writeFloat(v.y);
	}

	void writeVector3(const KBVector3f& v)
	{
		writeFloat(v.x);
		writeFloat(v.y);
		writeFloat(v.z);
	...
	void writeVector4(const KBVector4f& v)
	{
		writeFloat(v.x);
		writeFloat(v.y);
		writeFloat(v.z);
...
template<>
inline void MemoryStream::read_skip<KBString>()
{
	read_skip<char*>();
}
