...
void Bundle::send(std::shared_ptr<NetworkInterfaceBase> pNetworkInterface)
{
	fini(true);
...
void Bundle::appendBlob(const KBArray<uint8>& datas)
{
...
{
	uint32_t len = static_cast<uint32_t>(str.length()) + 4; // +4 保留原来的长度预留
...
void Bundle::writeVector2(const KBVector2f& v)
{
	checkStream(8);
	pCurrPacket_->writeVector2(v);
}

void Bundle::writeVector3(const KBVector3f& v)
{
	checkStream(12);
	pCurrPacket_->writeVector3(v);
}

void Bundle::writeVector4(const KBVector4f& v)
{
	checkStream(16);
