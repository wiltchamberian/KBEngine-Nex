...
public:
	...
	void writeMsgLength();
	void send(std::shared_ptr<NetworkInterfaceBase> pNetworkInterface);
...
public:
	...
	Bundle &operator<<(const KBString &value);
	...
	void writeUnicode(const KBString& v)
	{
		appendUTF8String(v);
	}
	
	void writeBlob(const KBArray<uint8>& v)
	{
		appendBlob(v);
	}

	void writePython(const KBArray<uint8>& v)
	{
		appendBlob(v);
	}
